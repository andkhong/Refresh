...
module.exports = {
  procImage: (req, res, next) => {
    ...
    // Jimp has async issues
    callJimp(bufferedImage, filter).then(jimp => 
      writeFile(jimp, token, mimetype)
        .then(() => res.send(token))
        .catch(err => console.log("Error with Step 2: writing file", err))
    );
  }
}
...
function callJimp(buffer, filter){
  return Jimp.read(buffer).then(image => {
    image.brightness(parseFloat(filter.brightness)/1000)
      ...
      // End Bad CODE
      return image;
    }).catch(err => console.log("Jimp has an issue with command, this is due to", err));
}
